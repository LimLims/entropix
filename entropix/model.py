...
def attention(x: jax.Array, 
    ...
    def add_mask(_):
        # Add the mask only if attn_mask is not None
    ...
    def conditional_add_mask(scores, attn_mask, cur_pos):
        # Check if attn_mask is None outside of add_mask
        if attn_mask is None:
            # Only check cur_pos in this case without adding a mask
            return lax.cond(cur_pos == 0, do_not_add_mask, lambda _: scores, operand=None)
        else:
            # Proceed as before when attn_mask is not None
